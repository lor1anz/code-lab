...
int main()
{
    nvtxRangePush("Vector Add No Async");
    ...
    cudaStreamCreateWithPriority(&s2, cudaStreamNonBlocking, mid);
    cudaStreamCreateWithPriority(&s3, cudaStreamNonBlocking, low);

    cudaMallocAsync(&d_a, size, s3);
    cudaMallocAsync(&d_b, size, s2);
    cudaMallocAsync(&d_c, size, s1);

    cudaMemcpyAsync(d_a, h_a, size, cudaMemcpyHostToDevice, s3);
    ...
    vector_add_gpu<<<num_blocks, BLOCK_SIZE, 0, s1>>>(d_a, d_b, d_c, N);
